...
function addGamesToPage(games) {
    ...
    // loop over each item in the data (games)
...
let unfundedCount = GAMES_JSON.reduce(
    (acc, game) => {return (game.pledged < game.goal) ? 1 + acc : acc;},
    0
);

// create a string that explains the number of unfunded games using the ternary operator
let fundedCount = GAMES_JSON.reduce(
    (acc, game) => {return (game.pledged >= game.goal) ? 1 + acc : acc;},
    0
);
...
const sortedGamesCopy = [...sortedGames];
let firstGame, secondGame;
[firstGame, secondGame] = [sortedGamesCopy[0], sortedGamesCopy[1]];
